...
### Fixed `[defect]`
- Corrected the OWASP LLM authority pin in `skills/post-commit-audit/SKILL.md` Phase 1 Step 3 + Step 6 Provenance Block template + `CHANGELOG.md`. v1.2.0 shipped with `"OWASP LLM Top 10 v1.1 (2024-10)"`, which was wrong on two counts: (a) v1.1 was the *2023* cycle's October-2023 update, not a 2024-10 publication; (b) the skill's actual framework mappings (per Phase 1 Step 3 description) target the 2025 release. Standardized on **"OWASP Top 10 for LLM Applications 2025 (published 2024-11)"** — the project's canonical name for the 2025 release. Caught by Codex P2 on cwe-mapper PR #2 ([commit 32700db](https://github.com/justice8096/cwe-mapper/commit/32700db)); sibling skills fixed in the same wave (sast-dast-scanner PR #2, post-commit-audit this release).
- **Downstream impact**: any AUDIT_SUMMARY.txt or LLM-Compliance report generated by post-commit-audit v1.2.0 between 2026-05-17 and now will carry the wrong OWASP LLM pin in its Provenance Block. Re-run audits against this v1.2.1 to refresh the pin if the audit is being relied on for legal/compliance evidence.
